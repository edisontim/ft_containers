...
class vector
{
	...
	public:
		...
		{
			//If the size reached the capacity of our array, we first need to reallocate some extra space and we copy its previous values inside it
			// before we can construct the new value in our array
			if (_size >= _capacity)
			{
				size_type	temp_cap;
